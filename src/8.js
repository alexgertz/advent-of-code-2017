let puzzleInput = `uz inc 134 if hx > -10
qin dec -300 if h <= 1
...
class Instruction {
    constructor (register, operation, condition) {
...
class Condition {
    constructor (register, operator, value) {
        this.register = register;
        this.operator = operator;
        this.value = parseInt(value);
    }

    isValid (compareValue) {
        return eval(compareValue + this.operator + this.value);
    }
}
...
class Registry {
    ...
    findMax () {
        return Math.max(...Object.values(this).filter(a => !isNaN(a)));
    }
    ...
    init (arr) {
        ...
        arr.forEach(input => {
            ...
            let operation = new Operation(...input.slice(1, 3));
            let condition = new Condition(...input.slice(4));

            this.instructions.push(new Instruction(register, operation, condition));
    ...
    runInstructions () {
        this.instructions.forEach(instruction => {
            let hasMetCondition = instruction.condition.isValid(this[instruction.condition.register]);
            if (hasMetCondition) {
                this[instruction.register] = this[instruction.register] + instruction.operation.getValue();
                
                if (this[instruction.register] > this.max.value) {
                    this.max.value = parseInt(this[instruction.register]);
                }
            }
        });
    }
...
let register = new Registry(puzzleArray);
register.runInstructions();

console.log("Part one: " + register.findMax());
console.log("Part two: " + register.max.value);
