var puzzleInput = `nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
const isValidPassphrase = (passphrase, operator) => {
    passphraseArray = passphrase.split(' ');

    if (operator === 'anagram') {
        passphraseArray = passphraseArray.map(a => a.split('').sort().join(''));
    }

    let uniqueWords = passphraseArray.filter((word, index) => {
        return passphraseArray.lastIndexOf(word) == index;
    });

    return uniqueWords.length == passphraseArray.length;
};


let validPassphrasesPartOne = puzzleArray.filter(passphrase => isValidPassphrase(passphrase, 'simple'));
let validPassphrasesPartTwo = puzzleArray.filter(passphrase => isValidPassphrase(passphrase, 'anagram'));
console.log("Part one: " + validPassphrasesPartOne.length);
console.log("Part two: " + validPassphrasesPartTwo.length);