...
var puzzleArray = puzzleInput.split('\n');

// To find the bottom tower we need a tower that is not a subtower
let towerRows = puzzleArray.map(row => row.split(/[^a-zA0-9]+/));
let subTowers = towerRows.map(row => row.slice(2)).
                    reduce((a,b) => { return a.concat(b)});

let bottomTower = towerRows.filter(row => !subTowers.includes(row[0]));
console.log("Part one: " + bottomTower);
